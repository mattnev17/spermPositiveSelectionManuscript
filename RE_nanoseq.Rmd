...
exomeVars_dNdS <- read_tsv(paste0(dndsPath, "inputFor_dNdS/mutationsInput/exomeVars_dNdS.tsv"), col_types = cols())

dndsBasic <- dndscv(exomeVars_dNdS, max_muts_per_gene_per_sample = Inf, max_coding_muts_per_sample = Inf,
                            outmats = T, gene_list = names(vCovEx))
...
compGlobal_dNdS <- dndsBasic$globaldnds  |> mutate(test = "Basic") |> 
  bind_rows(read_tsv(paste0(dndsPath, "inputFor_dNdS/results/globaldnds_TriExomeAll.tsv"), col_types = cols()) |> 
            mutate(test = "BasePairCov")) |> 
  bind_rows(read_tsv(paste0(dndsPath, "inputFor_dNdS/results/globaldnds_TriMethExomeAll.tsv"), col_types = cols()) |> 
          mutate(test = "BasePairCov + CpGmeth")) |> 
  bind_rows(read_tsv(paste0(dndsPath, "inputFor_dNdS/results/globaldnds_PentaMethExomeAll.tsv"), col_types = cols()) |> 
...
# Read the datasets and combine them with NS for missing values
# compGene_dNdSA <- read_tsv(paste0(dndsPath, "inputFor_dNdS/results/sel_cv_TriCombAll.tsv"), col_types = cols()) |> 
#   mutate(test = "BasePairCov") |> 
...
p_compGene_dNdScomb <- ggplot(compGene_dNdScomb, aes(x = test, y = gene_name, fill = Significant)) +
...
png(paste0(paperPath, 'extended/FigExt4_dndsModel.png'), height=11, width=13, res=300, units = "in")
p_covLevel + p_methLevel + p_methMutRate +  p_compGlobal_dNdS + p_compGene_dNdScomb + plot_layout(design = layout) + plot_annotation(tag_levels = "a")
dev.off()
pdf(paste0(paperPath, 'extended/FigExt4_dndsModel.pdf'), height=11, width=13, onefile = F)
p_covLevel + p_methLevel + p_methMutRate +  p_compGlobal_dNdS + p_compGene_dNdScomb + plot_layout(design = layout) + plot_annotation(tag_levels = "a")
dev.off()
```
